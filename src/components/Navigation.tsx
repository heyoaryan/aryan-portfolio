        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center group">
            <div className="relative">
              <Code className="w-7 h-7 sm:w-8 sm:h-8 text-blue-400 group-hover:rotate-180 transition-transform duration-500" />
              <div className="absolute inset-0 bg-blue-400 rounded-full animate-ping opacity-20 group-hover:opacity-40"></div>
            </div>
            <span className="ml-3 text-xl font-bold text-white group-hover:text-blue-400 transition-colors duration-300 hidden sm:block">
              Aryan Singh Thakur
            </span>
            <span className="ml-3 text-lg font-bold text-white group-hover:text-blue-400 transition-colors duration-300 sm:hidden">
              Aryan
            </span>
          </div>
          ...
          <div className="lg:hidden flex items-center">
            {currentPage === 'home' && (
              <button
                ...
                className="group relative p-2 text-gray-300 hover:text-white focus:outline-none focus:text-white transition-all duration-300 hover:bg-slate-800/50 rounded-lg bg-slate-800/30 backdrop-blur-sm border border-slate-700/50"
              >
                <div className="relative">
                  {isOpen ? (
            ...
            <div className="px-4 pt-2 pb-3 space-y-1">
              {navItems.map(({ name, href, icon: Icon }, index) => (
                <button
                  key={name}
                  onClick={() => handleNavClick({ name, href })}
                  className={`group flex items-center space-x-3 px-4 py-3 rounded-lg text-base font-medium transition-all duration-300 transform w-full ${
                    activeSection === name.toLowerCase()
